...
public class ItemsStAXBuilder extends AbstractItemBuilder {
	...
	private Plane buildPlane(XMLStreamReader reader) throws XMLStreamException {
		...
		plane.setId(reader.getAttributeValue(null, PlaneEnum.ID.getValue()));
		String name;
		while (reader.hasNext()) {
			int type = reader.next();
			switch (type) {
	...
	private Engine getXMLEngines(XMLStreamReader reader) throws XMLStreamException {
		...
		int amount = Integer.parseInt(reader.getAttributeValue(null, PlaneEnum.AMOUNT.getValue()));
		...
		while (reader.hasNext()) {
			...
			case XMLStreamConstants.END_ELEMENT:
				name = reader.getLocalName();
				if (PlaneEnum.valueOf(name.toUpperCase()) == PlaneEnum.ENGINES) {
					return engine;
				}
				break;
			}
	...
	private Parameters getXMLParameters(XMLStreamReader reader) throws XMLStreamException {
		...
		params.setColor(reader.getAttributeValue(null, PlaneEnum.COLOR.getValue()));
		boolean radar = Boolean.valueOf(reader.getAttributeValue(null, PlaneEnum.RADAR.getValue()));
